...
--z =============================================
--exec [SpRptGetAllInactiveStoreForExcel] 0,0,0,0,0,0,999,1
CREATE PROCEDURE [dbo].[SpRptGetAllInactiveStoreForExcel] 
@NodeID INT=0,
@NodeType INT=0,
@PNodeID INT=0,
@PNodeType INT=0,
...
BEGIN
	...
	--SELECT * FROM #RsltRouteList WHERE RouteId=45051
	...
	IF @ChannelId=1 --GT
	BEGIN
		SELECT StoreID INTO #InCoverage FROM tblRouteCoverageStoreMapping R WHERE CAST(GETDATE() AS DATE) BETWEEN FromDate AND ToDate

		;WITH RESULT AS
		(
		SELECT StoreID,RouteID,RouteNodeType,ROW_NUMBER() OVER(PARTITION BY StoreID ORDER BY Fromdate DESC) Rn
		FROM tblRouteCoverageStoreMapping R WHERE ToDate<CAST(GETDATE() AS DATE)
		)
		SELECT * INTO #Result FROM RESULT
		--SELECT * FROM #Result WHERE StoreID=27
		SELECT StoreID,RouteID,RouteNodeType INTO #tblOldRouteCoverageStoreMapping FROM #Result WHERE rn=1

		--SELECT * FROM #tblOldRouteCoverageStoreMapping
		...
		INNER JOIN #tblOldRouteCoverageStoreMapping A ON A.RouteID=R.RouteId AND A.RouteNodeType=R.RouteNodeType AND A.StoreID=SM.StoreID
		...
		WHERE CASE @flgStoreType WHEN 999 THEN 0 ELSE  @flgStoreType END =ISNULL(flgValidated,0)	
