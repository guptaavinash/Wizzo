...
CREATE PROCEDURE [dbo].[SpGetTCOrderForSFA] 
	@PDACOde VARCHAR(100),
...
BEGIN
	...
	SELECT O.StoreID,MAX(O.OrderID) MaxOrderID,MAX(T.TeleCallingId) TeleCallingId INTO #LastTCOrder FROM tblTCOrderMaster O INNER JOIN [dbo].[tblTeleCallerListForDay] T ON T.TeleCallingId=O.TeleCallingId INNER JOIN #DSRStoreList S ON S.StoreID=O.StoreID WHERE T.flgCallConversionStatus=1 GROUP BY O.StoreID
	...
	--SELECT * FROM @tblRawDataInvoiceHeader WHERE StoreID=89362
