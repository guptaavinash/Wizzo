...
--[spRptGetBasicDayTracker_PreOrder]'07-Apr-2022',4492,''
CREATE PROC [dbo].[spRptGetBasicDayTracker_PreOrder_Check]
@strDate DATE,
@LoginId INT,
...
AS
	...
	*/
	...
	--SELECT *	FROM [#VIsitedStores] WHERE SalesmanId=3 ORDER BY CovAreaId,DeviceVisitStartTS
	...
	*/
	...
	SELECT VM.RouteID,VM.RouteType,VM.StoreID,'On Behalf Working' ActualWorking,EntryPersonNodeID,EntryPersonNodeType,P.Descr Personname,RH.CovAreaNodeID CovAreaNodeID ,RH.CovAreaNodeType CovAreaNodeType INTO #ExternalWorking FROM #tmpRsltWithFullHierarchy H INNER JOIN tblVisitMaster VM ON VM.RouteID=H.RouteId AND VM.RouteType=H.RouteNodeType INNER JOIN tblRoutePlanningVisitDetail RH ON RH.RouteNodeId=VM.RouteID AND RH.RouteNodetype=VM.RouteType AND RH.VisitDate=@strDate INNER JOIN tblMstrPerson P ON P.NodeID=VM.EntryPersonNodeID AND P.NodeType=VM.EntryPersonNodeType WHERE VM.VisitDate=CAST(@strDate AS DATE) AND SalesmanNodeId<>EntryPersonNodeID

	--SELECT * FROM #ExternalWorking
	...
	SELECT DISTINCT ZoneId,ZoneNodeType,Zone,ASMAreaId,ASMAreaNodeType,ASMArea,SOAreaId,SOAreaNodeType,SOArea + ' (' + ISNULL(Salesman,'VACANT') + ')',CovAreaId,CovAreaNodeType,CovArea,SalesmanNodeId,SalesmanNodeType, Salesman
	...
	UPDATE A SET A.Activity=E.ActualWorking + ' By ' + Personname FROM #Final A INNER JOIN #ExternalWorking E ON E.CovAreaNodeID=A.CoverageAreaId AND E.CovAreaNodeType=A.CoverageAreaNodeType

	--SELECT * FROM #Final WHERE CoverageAreaId=2
