...
-- =============================================
--exec [spGetStoreDetailsByDSR_NEw_tocheck1]54,130,63,'27-Jan-2022'
CREATE PROCEDURE [dbo].[spGetStoreDetailsByDSR_New_tocheck1]
@SEAreaId INT,
@SEAreaNodeType INT,
@SalesmanId INT,
@Date DATE
...
BEGIN
	...
	SELECT DISTINCT VD.VisitDetID,VM.VisitId,VM.RouteId,VM.RouteType,VM.StoreId,VM.VisitLatitude,VM.VisitLongitude,VM.FlgOnRoute,ISNULL(VD.VisitStartDate,DeviceVisitStartTS) VisitStartDate,ISNULL(VD.VisitEndDate,DeviceVisitEndTS) VisitEndDate,flgTelephonicCall,IsGeoValidated,VM.Accuracy INTO #tblVisitMaster 
	FROM tblVisitMaster VM LEFT OUTER JOIN tblVisitDet VD ON VD.VisitID=VM.VisitID
	...
	SELECT DISTINCT OM.OrderID,VM.StoreId,V.CategoryNodeID,OD.ProductID,CAST(ROUND((OD.OrderQty * V.UOMValue)/1000,0) AS FLOAT) [Order(Kg)],OrderQty [Order(Pcs)],CAST(ROUND(OrderQty/C.RelConversionUnits,2) AS FLOAT) [Order(Cases)],CAST(ROUND(OD.NetLineOrderVal,0) AS FLOAT) [Order(Value)]
	INTO #Orders
	FROM #Target VM
...
--CREATE TABLE #Target(ReportDate Date,CovAreaNodeID INT,CovAreaNodeType INT,[Coverage] VARCHAR(200),RouteNodeId INT,RouteNodeType INT,RouteName VARCHAR(200),StoreID INT,StoreName VARCHAR(200),PrcRegionID INT,[Lat Code] NUMERIC(26,22),[Long Code] NUMERIC(26,22),flgTargetCall TINYINT,flgActualVisit TINYINT,flgTelephonicVisit TINYINT,flgGeofenceValidated TINYINT,flgProductive TINYINT,flgOffRoute TINYINT DEFAULT 1,OrderID INT,[Order(Kg)] DECIMAL(18,2) DEFAULT 0,[Order(Cases)] DECIMAL(18,2) DEFAULT 0, OrderValue DECIMAL(18,2) DEFAULT 0,Distancefrompoint FLOAT)
	...
	SELECT * FROM #Target WHERE flgFirstVisit=1
	...
	CASE flgActualVisit WHEN 1 THEN 1 ELSE 0 END flgShowMap,CAST(OrderValue AS INT) [Sales Value],[Order(Cases)] [Order (Cases)],CAST([Order(Kg)] AS INT) [Order (Kg)]' + @StrCategory + ',ISNULL(flgActualVisit,0) flgActualVisit,ISNULL(flgTelephonicVisit,0) flgTelephonicVisit,[Lat Code] StoreLatitude,[Long Code] StoreLongitude,flgOffROute
