...
-- [SpForPDAGetPDAStoreSummary_tocheck] '1ADE3CEC-CFB6-4B66-9EC9-32E194761084'
CREATE PROCEDURE [dbo].[SpForPDAGetPDAStoreSummary_tocheck] 
	@PDACode VARCHAR(100),
...
BEGIN
	...
	SELECT * FROM #DSRStoreList WHERE StoreID=101383
	...
	;WITH CTE AS 
	(
		SELECT T.StoreID,TeleCallingId,ROW_Number() OVER(PARTITION BY T.StoreID Order BY TeleCallingId DESC) rn FROM [tblTeleCallerListForDay] T INNER JOIN #DSRStoreList S ON S.StoreID=T.StoreId WHERE T.flgCallConversionStatus=1
	)
	SELECT * INTO #TelecallingStores FROM CTE WHERE rn<=5

	SELECT * FROM #TelecallingStores WHERE SToreID=101383
