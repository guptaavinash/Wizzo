...
CREATE PROCEDURE [dbo].[SpRptGetAllSyncStoreForExcel] 
@NodeID INT=0,
@NodeType INT=0,
@PNodeID INT=0,
@PNodeType INT=0,
...
BEGIN
	...
	SELECT V.* INTO #tblRoutePlanningVisitDetail FROM (SELECT RouteNodeId,RouteNodetype,MAX(VisitDate) VisitDate FROM tblRoutePlanningVisitDetail GROUP BY RouteNodeId,RouteNodetype) M INNER JOIN tblRoutePlanningVisitDetail V ON M.RouteNodeId=V.RouteNodeId AND M.RouteNodetype=V.RouteNodetype AND M.VisitDate=V.VisitDate
	...
	--SELECT * FROM #tmpRsltWithFullHierarchy WHERE CompRouteId=48186
	...
	BEGIN
		...
		SELECT DISTINCT R.Region,R.ASMArea AS [ASM Area],R.SOArea AS [SO Area],[Coverage Area],RM.RouteCode,[Route],DBR.DistributorCode,DBR.Descr,P.Code,ISNULL(P.Descr,'Not COvered'),SM.StoreIdDB,SM.StoreId,SM.StoreCode,SM.StoreName AS [Store Name], ISNULL(REPLACE(REPLACE(StoreAdd.StoreAddress1,CHAR(13),''),CHAR(10),''),'') AS [Store Address], ISNULL(StoreAdd.City,'') AS City,ISNULL(CAST(StoreAdd.State AS VARCHAR),'') State,ISNULL(CAST(StoreAdd.Pincode AS VARCHAR),'') AS [Pin Code],ISNULL(StoreCont.FName,'') [Owner Name],ISNULL(CAST(StoreCont.MobNo AS VARCHAR),'''') [Contact No],
		ISNULL(V.SubChannel,'NA') AS [Store Category],ISNULL(I.StoreSegment,'NA'),SM.[Lat Code],SM.[Long Code],CASE ISNULL(StoreIDDB,0) WHEN 0 THEN 'Old' ELSE 'New' END,dbo.fncUtlDateString(SM.TimeStampIns) AS [Created Date],CASE ISNULL(SM.flgValidated,0) WHEN 0 THEN 'Approval Pending' WHEN 1 THEN 'Approved' WHEN 2 THEN 'Rejected' WHEN 3 THEN 'ReMap' END AS Status
		...
		LEFT OUTER JOIN tblSalespersonmapping PM ON PM.NodeID=R.CovAreaId AND PM.NodeType=R.CovAreaNodeType AND CAST(GETDATE() AS DATE) BETWEEN PM.FromDate AND PM.ToDate	
		LEFT OUTER JOIN tblMstrPerson P ON P.NodeID=PM.PersonNodeID 
		...
		SELECT Region,[ASM Area],[Coverage Area],DIstributorCode,DistributorName,RouteCode,Route,EmpCode,CoveredBy,StoreID,[Store Code],[Store Name],[Store Address],City,State,[Pin Code],[Owner Name],[Contact No],[Store Category],[Store Segment],[Lat Code],[Long Code],[New/Old],[Created Date],[FirstProductive] [FirstProductive Date],Status FROM #tblStoreList_GT 
