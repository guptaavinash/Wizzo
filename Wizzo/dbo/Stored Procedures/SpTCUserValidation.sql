...
create PROCEDURE [dbo].[SpTCUserValidation] 
...
BEGIN
	...
	SELECT @PersonNodeID=EmpId FROM tblTCEmpMstr WHERE ContactNo=CAST(@ContactNo AS VARCHAR)

	PRINT 'PersonNodeID=' + CAST(ISNULL(@PersonNodeID,0) AS VARCHAR)
	IF ISNULL(@PersonNodeID,0)>0
	BEGIN
		IF  NOT EXISTS(SELECT 1 FROM tblTCPDACodeMapping  WHERE PersonID=@PersonNodeID AND @asOf BETWEEN ValidFrom and ValidTo ) 
		BEGIN
			IF @flgStep=1
			BEGIN
				SELECT @PDACode=CONVERT(varchar(50), NEWID())

				INSERT INTO tblTCPDACodeMapping(PersonID,PDACode,ValidFrom,ValidTo,FCMToken)
		...
		ELSE 
		BEGIN
			SELECT @flgLock=flgLock FROM tblTCPDACodeMapping WHERE PersonID=@PersonNodeID AND CAST(@asOf AS DATE) = CAST(ValidFrom AS DATE)
			SELECT @OTPExists=OTP FROM tblTCPDACodeMapping WHERE PersonID=@PersonNodeID
			...
			BEGIN
				...
				BEGIN
					...
					INSERT INTO [tblTCPDACodeMapping_History](PersonID,PDACode,OTP,ValidFrom,ValidTo,FCMToken)
					SELECT PersonID,PDACode,OTP,ValidFrom,ValidTo,FCMToken FROM tblTCPDACodeMapping WHERE PersonID=@PersonNodeID

					UPDATE tblTCPDACodeMapping SET PDACode=@PDACode,ValidFrom=@asOf,FCMToken=@FCMToken FROM tblTCPDACodeMapping WHERE PersonID=@PersonNodeID
			...
			BEGIN
				...
				BEGIN
					SELECT @PDACode=PDACode FROM tblTCPDACodeMapping WHERE PersonID=@PersonNodeID
					UPDATE tblTCPDACodeMapping SET OTP=@OTP,flgLock=1,FCMToken=@FCMToken FROM tblTCPDACodeMapping WHERE PersonID=@PersonNodeID
