...
CREATE PROCEDURE [dbo].[SpToChangeRouteForStore] 
...
BEGIN
	...
	begin
		...
		select top 1 @CheckFromDt = FromDate from tblRouteCoverageStoreMapping where StoreID = @StoreID and GETDATE() between FromDate and ToDate 
			...
			BEGIN
						...
						insert into tblRouteCoverageStoreMapping values(@NewRouteID,@StoreID,cast(GETDATE()+1 as date),'31-Dec-2049',1,GETDATE(),NULL,NULL,140,NULL)
		...
		begin
			Print 'Invalid Details' --+ cast (@OldRouteID as varchar) + 'New Route ID - ' + cast(@NewRouteID as varchar) + 'Store id - ' + @StoreID
