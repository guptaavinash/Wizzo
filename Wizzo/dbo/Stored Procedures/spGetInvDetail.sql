CREATE proc [dbo].[spGetInvDetail] -- 2,1
...
select p.SKUCode AS ProductCode,P.Descr as  ProductName,A.OrderQty AS Quantity,A.FreeQty,'Pcs' as UOM,convert(numeric(18,2),A.ProductRate) as Rate,convert(numeric(18,2),A.NetLineOrderVal) AS [Sales Value] from tblTCOrderDetail A JOIN tblPrdMstrSKULvl p on a.PrdNodeId=p.NodeID where OrderID=@InvId
end
