...
-- SpGetRoutePlanCalender 'D7E14137-2F47-4BDD-A4C2-9B9924127A09'
CREATE PROCEDURE [dbo].[SpGetRoutePlanCalender_09Feb2022] 
	@PDA_IMEI VARCHAR(50),
...
BEGIN
	...
	INSERT INTO #Routes(Nodeid,Nodetype)
		SELECT DISTINCT  RouteNodeId,RouteNodetype FROM tblRoutePlanningVisitDetail(nolock) RP INNER JOIN #CoverageArea C ON C.NodeID=RP.CovAreaNodeID AND C.NodeType=RP.CovAreaNodeType INNER JOIN #TotalDates D ON D.MonthVal=MONTH(VisitDate) AND D.YearVal=YEAR(VisitDate)
	...
	INSERT INTO #tblRoutePlan(RouteNodeID,RouteNodeType,RouteName,PlannedDate,StoreNodeID,WeekID,MonthVal,MonthYear)
	SELECT DISTINCT H.NodeID RouteNodeID,H.NodeType RouteNodetype,R.Descr,D.Date,RM.StoreID,DATEPART(WEEK, D.Date)  -  DATEPART(WEEK, DATEADD(MM, DATEDIFF(MM,0,D.Date), 0))+ 1,D.MonthVal,LEFT(DATENAME(month,DATEADD(mm,D.MonthVal,-1)),3) + '-' + CAST(D.YearVal AS VARCHAR) FROM tblCompanySalesStructureHierarchy(nolock) H INNER JOIN #CoverageArea C ON H.PNodeID=C.NodeID AND H.PNodeType=C.NodeType 
	LEFT OUTER JOIN tblRouteCoverageStoreMapping(nolock) RM ON RM.RouteID=H.NodeID AND RM.RouteNodeType=H.NodeType
	--AND GETDATE() BETWEEN RM.FromDate AND RM.ToDate
	INNER JOIN #Routes RL ON RL.NodeID=RM.RouteID AND RL.NodeType=RM.RouteNodeType
	LEFT OUTER JOIN tblCompanySalesStructureRouteMstr(nolock) R ON R.NodeID=H.NodeID AND R.NodeType=H.NodeType
	--LEFT OUTER JOIN tblStoreMaster(nolock) SM ON SM.StoreID=RM.StoreID 
	CROSS JOIN #TotalDates D WHERE GETDATE() BETWEEN H.VldFrom AND H.VldTo AND GETDATE() BETWEEN RM.FromDate AND RM.ToDate

	--SELECT * FROM #tblRoutePlan WHERE StoreNodeID IS NULL
	...
	INSERT INTO #Route(RouteNodeID,RouteNodetype,PlannedDate)
	SELECT DISTINCT NodeID,NodeType,V.VisitDate FROM #Routes S INNER JOIN tblRoutePlanningVisitDetail V ON V.RouteNodeId=S.NodeID AND V.RouteNodetype=S.NodeType
	...
	SELECT * FROM #Route WHERE RouteNodeID=50280
	...
	UPDATE A SET flgAttendance=1 FROM #tblRoutePlan A WHERE A.PlannedDate IN (SELECT DISTINCT AttnDate FROM #MarkedDates WHERE ReasonID IN (SELECT ReasonID FROM tblMstrReasonsForNoVisit WHERE flgNoVisitOption=0)) AND PlannedDate<=CAST(GETDATE() AS DATE)
	...
	--SELECT * FROM #tblRoutePlan
	SELECT * FROM #Route WHERE PlannedDate='10-Feb-2022'
