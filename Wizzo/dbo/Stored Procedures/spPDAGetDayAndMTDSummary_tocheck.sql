...
CREATE PROCEDURE [dbo].[spPDAGetDayAndMTDSummary_tocheck]
@strDate VARCHAR(20), 
...
AS
	...
	--BEGIN  
		...
		BEGIN
			...
			FROM tblSalesPersonMapping SP INNER JOIN [tblSecMenuContextMenu] C ON C.NodeType=SP.NodeType 
			INNER JOIN #PersonList P ON SP.PersonNodeId=P.PersonNodeId AND SP.PersonType=P.PersonNodeType
			WHERE ISNULL(C.flgRoute,0)=1 AND (GETDATE() BETWEEN SP.FromDate AND ISNULL(SP.ToDate,GETDATE())) 
	...
	BEGIN
		...
		SELECT OM.VisitId,OM.StoreID,VM.FlgOnRoute,OM.OrderDate,OD.ProductID,OD.OrderQty,CAST(OD.OrderQty AS FLOAT)* Hier.Grammage AS OrderQtyInKG,Hier.CategoryNodeId,Hier.Category, Hier.CatOrdr
	...
	*/
	...
	-- AND YEAR(OrderDate)=YEAR(GETDATE())
	 ...
	 SELECT @TotalMonthSecSales=SUM(OrderQtyInKG)/1000 FROM #Orders
	 SELECT @TodaysSecSales=SUM(OrderQtyInKG)/1000 FROM #Orders WHERE OrderDate=CAST(GETDATE() AS DATE)
