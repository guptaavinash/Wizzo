...
--[spRptGetBasicDayTracker_PreOrder_FullMonth]'09-Feb-2022',4492,''
CREATE PROC [dbo].[spRptGetBasicDayTracker_PreOrder_FullMonth_tocheck]
@strDate DATE,
@LoginId INT,
...
AS	
	...
	UPDATE A SET A.SalesmanNodeId=MP.NodeId,A.SalesmanNodeType=Mp.NodeType,A.Salesman=ISNULL(MP.Descr,'Vacant'),A.[User Contact]=MP.PersonPhone,A.[User Status]=CASE WHEN MP.flgActive=1 THEN 'Yes' ELSE 'No' END
	...
	--SELECT * FROM #tmpRsltWithFullHierarchy WHERE SalesmanNodeId iN (220,173)
	...
	SELECT DISTINCT R.CovAreaId,R.CovAreaNodeType,R.ASMAreaId,R.ASMAreaNodeType,R.RouteId,R.RouteNodetype,RCM.StoreId,0 AS AddedToday,ISNULL(SM.CreatedDate,'01-Nov-2021') CreatedDate  INTO #StoreList
	FROM #tmpRsltWithFullHierarchy R INNER JOIN tblRouteCoverageStoreMapping(nolock) RCM ON R.RouteId=RCM.RouteId AND R.RouteNodetype=RCM.RouteNodetype
	INNER JOIN tblStoreMaster(nolock) SM ON RCM.StoreId=SM.StoreId
	WHERE (GETDATE() BETWEEN RCM.FromDate AND RCM.ToDate)
	...
	*/
	...
	--Actual Calls
	SELECT R.CovAreaId,R.CovAreaNodeType,R.ASMAreaId,R.ASMAreaNodeType,V.VisitId,V.VisitDate,V.RouteID,V.RouteType AS RouteNodeType,V.StoreID,V.BatteryLeftStatus,ISNULL(VD.VisitStartDate,DeviceVisitStartTS) DeviceVisitStartTS,ISNULL(VD.VisitEndDate,DeviceVisitEndTS) DeviceVisitEndTS,DATEDIFF(ss,ISNULL(VD.VisitStartDate,V.DeviceVisitStartTS) ,ISNULL(VD.VisitEndDate,V.DeviceVisitEndTS)) TimeSpentInStore,ISNULL(V.EntryPersonNodeId,V.SalesPersonId) SalesmanId,V.flgTelephonicCall INTO [#VIsitedStores]
	...
	ORDER BY V.StoreID
	--SELECT *	FROM [#VIsitedStores] WHERE CovAreaId=153 AND VisitDate='22-Feb-2022' ORDER BY CovAreaId,DeviceVisitStartTS
	...
	*/
	...
	INNER JOIN #DayStartActivity C ON B.SalesmanNodeId=C.PersonNodeId AND C.AttenDate=A.VisitDate
	...
	INSERT INTO #Final(RptDate,RSMAreaId,RSMAreaNodeType,[RSM Area],StateHeadAreaNodeID,StateHeadAreaNodeType,StateHeadArea,ASMAreaId,ASMAreaNodeType,[ASM Area],SOAreaId,SOAreaNodeType,[SO Area],CoverageAreaId,CoverageAreaNodeType,CoverageArea, SalesmanNodeId,SalesmanNodeType,Salesman,[User Status],[User Contact])
	SELECT DISTINCT D.Date,ZoneId,ZoneNodeType,Zone,RegionNodeId,RegionNodeType,Region,ASMAreaId,ASMAreaNodeType,ASMArea,SOAreaId,SOAreaNodeType,SOArea,CovAreaId,CovAreaNodeType,CovArea,SalesmanNodeId,SalesmanNodeType, Salesman ,[User Status],[User Contact]
	FROM #tmpRsltWithFullHierarchy CROSS JOIN #tblDates D  WHERE ISNULL(RouteId,0)>0

	--SELECT * FROM #Final WHERE SalesmanNodeId IN (173,220)
	...
	UPDATE F SET ReasonDescr='P' FROM #Final F WHERE ReasonID IN (8,9,10,16) AND ReasonID IN (6,4,7,12,14,17)
	...
	UPDATE F SET AssignRoute= COALESCE(AssignRoute+',' , '') + Route FROM #Final F INNER JOIN #Target T ON T.CovAreaId=F.CoverageAreaId AND T.CovAreaNodeType=F.CoverageAreaNodeType AND T.RptDate=F.RptDate
	...
	UPDATE F SET [Calls^Avg Retailing Time(min:Sec)]= CAST(FLOOR([Calls^Avg Retailing Time(Sec)]/60) AS VARCHAR) + ':' + CAST([Calls^Avg Retailing Time(Sec)] % 60 AS VARCHAR) FROM #Final F 
	...
	SELECT @strSQL='SELECT SOAreaID,RptDate,[RSM Area] ,[ASM Area] ,[SO Area] ,AssignRoute,VisitRoute,CASE WHEN AssignRoute=VisitRoute THEN ''Yes'' ELSE ''No'' END AS [PJP Adherence],FORMAT(CAST(DOJ AS DATE),''dd-MMM-yy'') [Date Of Joining],[Start Time] ,[End Time] ,[Working Hours] [Working Hours] ,[Calls^Avg Retailing Time(min:Sec)] [Avg Retailing Time(min:Sec)],[Calls^Retailing Grade] [Retailing Grade],[Time Spent in Store] ,[Calls^Target] [Schedule Calls],[Calls^Actual] [Total Calls],[Calls^Productive] [Productive Shops],[Calls^Productivity %][Productivity %]' + @StrCategory + ',[Total Sales^Value] [Sales Value]
	...
	CREATE TABLE #RptAttendance(RSMAreaId INT,RSMAreaNodeType INT,[RSM Area] VARCHAR(200),StateHeadAreaNodeID INT,StateHeadAreaNodeType SMALLINT,StateHeadArea VARCHAR(200),ASMAreaId INT,ASMAreaNodeType INT,[ASM Area] VARCHAR(200),SOAreaId INT,SOAreaNodeType INT,[SO Area] VARCHAR(200),CoverageAreaId INT,CoverageAreaNodeType INT,CoverageArea VARCHAR(200),SalesmanNodeId INT,SalesmanNodeType INT,Salesman VARCHAR(200),[User Status] VARCHAR(20),DOJ VARCHAR(20),[User Contact] BIGINT,[Total Days] INT,[Working Days] INT DEFAULT 0,[Retailing Days(R)] INT DEFAULT 0,[Other Official Work (O)] INT DEFAULT 0,[Leave(L)] INT DEFAULT 0,[Holiday(H)] INT DEFAULT 0,[Absent(A)] INT DEFAULT 0,[Weekly Off(W)] INT DEFAULT 0,
	[Calls^Avg Retailing Time(min:Sec)] VARCHAR(10))
	...
	INSERT INTO #RptAttendance(RSMAreaId,RSMAreaNodeType,[RSM Area],StateHeadAreaNodeID,StateHeadAreaNodeType,StateHeadArea,ASMAreaId,ASMAreaNodeType,[ASM Area],SOAreaId,SOAreaNodeType,[SO Area],CoverageAreaId,CoverageAreaNodeType,CoverageArea,SalesmanNodeId,SalesmanNodeType,Salesman,[User Status],DOJ,[User Contact],[Calls^Avg Retailing Time(min:Sec)])
	SELECT DISTINCT RSMAreaId,RSMAreaNodeType,[RSM Area],StateHeadAreaNodeID,StateHeadAreaNodeType,StateHeadArea,ASMAreaId,ASMAreaNodeType,[ASM Area],SOAreaId,SOAreaNodeType,[SO Area],CoverageAreaId,CoverageAreaNodeType,CoverageArea,SalesmanNodeId,SalesmanNodeType,Salesman,[User Status],DOJ,[User Contact],[Calls^Avg Retailing Time(min:Sec)] FROM #Final
	...
	UPDATE R SET [Working Days]=WorkingDays FROM #RptAttendance R,(SELECT SalesmanNodeID,COUNT(RptDate) WorkingDays FROM #Final F WHERE ReasonDescr IN ('O','P') GROUP BY SalesmanNodeID) F WHERE F.SalesmanNodeId=R.SalesmanNodeId
	UPDATE R SET [Retailing Days(R)]=RetailingDays FROM #RptAttendance R,(SELECT SalesmanNodeID,COUNT(RptDate) RetailingDays FROM #Final F WHERE ReasonDescr IN ('P') GROUP BY SalesmanNodeID) F WHERE F.SalesmanNodeId=R.SalesmanNodeId
