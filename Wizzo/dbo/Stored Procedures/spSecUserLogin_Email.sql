...
--spSecUserLogin 'tc001','12345','ss','ss','ss','ss'
create  PROCEDURE [dbo].[spSecUserLogin_Email] 
...
AS
	...
	datediff(minute,CurrentActiveTime,dbo.fnGetCurrentDateTime()) else 10 end,@flgReleasingForTesting=flgReleasingForTesting FROM  tblSecUser WHERE     (UserMail = @UserName) AND Active=1
	
	-- CLOSE SYMMETRIC KEY System_password;
	IF @UserID>0
		BEGIN
			
			--if @RoleId not in (6,7,8,9,11)
			--begin
				...
				BEGIN
					...
					ELSE
						BEGIN
							IF @SameUser=1
								BEGIN
									UPDATE tblSecUserLogin SET IsSessionEnd=1, Logouttime=dbo.fnGetCurrentDateTime(), LogOutSrc=2 WHERE LoginID=@LoginID
			...
			IF @SameUser=0 --This means that there were no old records for user id in login table so a newentry can now be made
				BEGIN
					...
					INSERT INTO tblSecUserLogin  (UserID, SessionID, IPAddress, IEVersion, ScrRsltn,LoginTime)  VALUES     (@UserID, @SessionIDNw, @IPAddress,  @BrwsrVer, @ScrRsltn,dbo.fnGetCurrentDateTime())
					SET @LoginId=@@IDENTITY
					SET @LoginRslt=3
					UPDATE tblsecuser set PwdStatus=1,CurrentActiveTime=dbo.fnGetCurrentDateTime() where userid=@UserID
				END
		END
	ELSE
		BEGIN
		select 10 as CurrentLastActiveTimeInMin where 1<>1
			SET @LoginRslt=1
		END
		DECLARE @dATE DATE=dbo.fnGetCurrentDateTime(),@FYID int=0
--SELECT @FYID=FYID from tblFinancialYear where @dATE between FYStartDate and FYEndDate 

SELECT @TokenNo= C.FCMTokenNo
	FROM tblPDA_UserMapMaster A INNER JOIN tblPDAMaster B ON A.PDAId=B.PDAId 
	INNER JOIN [dbo].[tblEmpMstr] C ON A.EmpID=C.EmpId
...
Declare @TCDailyId int=0,@StatusId int=0,@currdate date=dbo.fnGetCurrentDateTime()
