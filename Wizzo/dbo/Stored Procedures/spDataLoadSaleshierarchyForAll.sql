...
Create proc [dbo].[spDataLoadSaleshierarchyForAll]
as
begin
...
update p set Descr=a.UserName,Designation=a.UserDesignation,PersonEmailID=a.Email,PersonPhone=a.Phone,flgActive=case when a.UserStatus='Active' then 1 else 0 end from #SalesHierarchy a  join tblMstrPerson p on a.UserErpId=p.Code and p.NodeType=case userrank when 0 then 220
when 1 then 210
when 2 then 200
when 3 then 195 end
