...
--select * from tblTeleCallerListForDay where Date='27-aug-2021' and tcnodeid=1 and isused=1
--exec [spGetOrderProductList] 70011,7648
CREATE proc [dbo].[spGetOrderProductList_LastDev] 
@StoreId int,
@TeleCallingId int
as
begin
...
set @StateId=0
select * INTO #SBFPrice from [dbo].tblprdskusalesmapping(nolock) where PrcLocationId=@PrcRegionId
...
ORDER BY CategoryNetValue DESC,Category,brandNetValue desc,OrderType,SBDGroup,flgBaseProduct DESC,PrdNetValue DESC
 ...
 select b.InvId,b.InvNo,format(b.InvDate,'dd-MMM') as [Inv Date],sum(NetValue) as NetValue,b.InvDate into #Top5Inv  from #InvDt a join #StoreLastOrderDetail b on a.InvDate=b.invdate

 group by b.InvId,b.InvNo,b.InvDate
 order by b.invdate desc
 select InvId,a.InvNo,a.[Inv Date],NetValue,case Row_number() over(order by a.invdate desc) when 1 then 1 else 0 end  as flgTeleOrderInv,0.00 as  OutstandingAmt from #Top5Inv a 
...
DSEIssueIds,(select count(*) from #TmpCrossTab where sectorid=a.sectorid and flgSP=1) AS flgSP,IsDiscountApplicable,pr.PrcRegion,DialerTypeId
...
select @LastOrderedDate=a.FAOrderDate,@LastOrderedValue=a.NetOrderValue from tblfaordermaster a  where FAOrdId=@InvId
