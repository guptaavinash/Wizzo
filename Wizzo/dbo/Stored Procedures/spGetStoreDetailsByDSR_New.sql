...
-- =============================================
--exec [spGetStoreDetailsByDSR_NEw]109,130,138,'22-Feb-2022'
CREATE PROCEDURE [dbo].[spGetStoreDetailsByDSR_New]
@SEAreaId INT,
@SEAreaNodeType INT,
@SalesmanId INT,
@Date DATE
...
BEGIN
	...
	UPDATE R SET flgPlanned=1 FROM #RouteList R INNER JOIN tblRoutePlanningVisitDetail V ON V.RouteNodeId=R.RouteId AND V.RouteNodetype=R.RouteNodeType AND V.VisitDate=@Date AND V.CovAreaNodeID=@SEAreaId AND V.CovAreaNodeType=@SEAreaNodeType
	...
	CREATE TABLE #Tmp(VisitDetID INT,RouteNodeId INT,RouteNodeType INT,RouteName VARCHAR(200),StoreId INT,StoreName NVARCHAR(500),StoreAddress VARCHAR(200),VisitLatitude DECIMAL(27,24),VisitLongitude DECIMAL(27,24),
	...
	CREATE TABLE #Target(ReportDate Date,CovAreaNodeID INT,CovAreaNodeType INT,[Coverage] VARCHAR(200),RouteNodeId INT,RouteNodeType INT,RouteName VARCHAR(200),StoreID INT,StoreName VARCHAR(200),StoreAddress VARCHAR(200),PrcRegionID INT,[Lat Code] NUMERIC(26,22),[Long Code] NUMERIC(26,22),VisitLatitude DECIMAL(27,24),VisitLongitude DECIMAL(27,24),StartTime TIME,TimeSpent TIME,TimeGap VARCHAR(10),flgTargetCall TINYINT,flgActualVisit TINYINT,flgTelephonicVisit TINYINT,flgGeofenceValidated TINYINT,flgProductive TINYINT,flgOffRoute TINYINT DEFAULT 1,OrderID INT,[Order(Kg)] DECIMAL(18,2) DEFAULT 0,[Order(Cases)] DECIMAL(18,2) DEFAULT 0,[Order(Pcs)] DECIMAL(18,2) DEFAULT 0, OrderValue DECIMAL(18,2) DEFAULT 0,Distancefrompoint FLOAT,GapfromPrvVisitInSec INT,Accuracy FLOAT,LinesOrdered INT,flgFirstVisit TINYINT,VisitedRoute VARCHAR(200))

	INSERT INTO #Target(ReportDate,CovAreaNodeID,CovAreaNodeType,RouteNodeId,RouteNodeType,RouteName,StoreID,StoreName,StoreAddress,PrcRegionID,[Lat Code],[Long Code],flgTargetCall)
	SELECT DISTINCT @Date AS ReportDate,@SEAreaId,@SEAreaNodeType,#RouteList.RouteId, #RouteList.RouteNodeType ,#RouteList.RouteName ,AA.StoreID AS StoreID,SM.StoreName,A.StoreAddress1,PR.PrcRgnNodeId,SM.[Lat Code],SM.[Long Code],flgPlanned 
	...
	LEFT OUTER JOIN tblOutletAddressDet A ON A.StoreID=SM.StoreID
	...
	INSERT INTO #Tmp(VisitDetID,RouteNodeId,RouteNodeType,RouteName,StoreId,StoreName,StoreAddress,VisitLatitude,VisitLongitude,StartTime,EndTime,TimeSpent,ActLatCode,ActLongCode,flgTelephonicCall,IsGeoValidated,Accuracy,Seq)	
	SELECT DISTINCT VisitDetID,VM.RouteID,VM.RouteType,VM.RouteName,SM.StoreId,SM.StoreName AS StoreName,A.StoreAddress1,VM.VisitLatitude,VM.VisitLongitude, VisitStartDate,VisitEndDate,CONVERT(VARCHAR,((CONVERT(DATETIME,VisitEndDate,114) - CONVERT(DATETIME,VisitStartDate,114))),114) AS TimeSpent,SM.[Lat Code],SM.[Long Code],flgTelephonicCall,IsGeoValidated,Accuracy,DENSE_RANK() OVER (ORDER BY VisitStartDate) 
...
--CREATE TABLE #Target(ReportDate Date,CovAreaNodeID INT,CovAreaNodeType INT,[Coverage] VARCHAR(200),RouteNodeId INT,RouteNodeType INT,RouteName VARCHAR(200),StoreID INT,StoreName VARCHAR(200),PrcRegionID INT,[Lat Code] NUMERIC(26,22),[Long Code] NUMERIC(26,22),flgTargetCall TINYINT,flgActualVisit TINYINT,flgTelephonicVisit TINYINT,flgGeofenceValidated TINYINT,flgProductive TINYINT,flgOffRoute TINYINT DEFAULT 1,OrderID INT,[Order(Kg)] DECIMAL(18,2) DEFAULT 0,[Order(Cases)] DECIMAL(18,2) DEFAULT 0, OrderValue DECIMAL(18,2) DEFAULT 0,Distancefrompoint FLOAT)
	...
	INSERT INTO #Target(ReportDate,CovAreaNodeID,CovAreaNodeType,RouteNodeId,RouteNodeType,RouteName,StoreID,StoreName,PrcRegionID,[Lat Code],[Long Code],flgTargetCall,flgActualVisit,flgTelephonicVisit,flgGeofenceValidated,Distancefrompoint,StartTime,TimeSpent,VisitLatitude,VisitLongitude,Accuracy,flgFirstVisit,TimeGap,VisitedRoute,flgOffRoute,StoreAddress)

	SELECT T.ReportDate,T.CovAreaNodeID,T.CovAreaNodeType,T.RouteNodeId,T.RouteNodeType,T.RouteName,T.StoreID,T.StoreName,T.PrcRegionID,T.[Lat Code],T.[Long Code],T.flgTargetCall,CASE flgTelephonicCall WHEN 1 THEN 0 ELSE 1 END,          V.flgTelephonicCall,V.IsGeoValidated,V.DistanceFromOrgPointInM,V.StartTime,V.TimeSpent,V.VisitLatitude,V.VisitLongitude,V.Accuracy,DENSE_RANK() OVER (PARTITION BY V.StoreID ORDER BY V.Starttime) Seq,dbo.fnGetTimeStringin_MinSecFormat(V.TimeGap),V.RouteName,T.flgOffRoute,T.StoreAddress FROM #TargetTemp T INNER JOIN #Tmp V ON V.StoreId=T.StoreID ORDER BY StartTime

	--SELECT * FROM #Target

	INSERT INTO #Target(ReportDate,CovAreaNodeID,CovAreaNodeType,RouteNodeId,RouteNodeType,RouteName,StoreID,StoreName,PrcRegionID,[Lat Code],[Long Code],flgTargetCall,VisitedRoute,flgOffRoute)
	SELECT T.ReportDate,T.CovAreaNodeID,T.CovAreaNodeType,T.RouteNodeId,T.RouteNodeType,T.RouteName,T.StoreID,T.StoreName,T.PrcRegionID,T.[Lat Code],T.[Long Code],T.flgTargetCall,T.VisitedRoute,T.flgOffRoute FROM #TargetTemp T WHERE StoreID NOT IN (SELECT StoreID FROM #TargetTemp)
	...
	DECLARE @DayStart Datetime,@DayStartLatCode NUMERIC(27,24),@DayStartLongCode NUMERIC(27,24),@DayStartAddress VARCHAR(200)
	,@DayEnd Datetime,@DayEndLatCode NUMERIC(27,24),@DayEndLongCode NUMERIC(27,24),@DayEndAddress VARCHAR(200)

	SELECT @DayStart=Datetime,@DayStartLatCode=[Lat Code],@DayStartLongCode=[Long Code],@DayStartAddress=Address FROM tblPersonAttendance WHERE PersonNodeID=@SalesmanId AND CAST(DAtetime AS DATE)=@Date

	SELECT @DayEnd=Endtime,@DayEndLatCode=LatCode,@DayEndLongCode=LongCode,@DayEndAddress=Address FROM tblDayEndDetails WHERE PersonId=@SalesmanId AND CAST(ForDate AS DATE)=@Date AND LatCode>0
	SELECT @DayEnd=Endtime FROM tblDayEndDetails WHERE PersonId=@SalesmanId AND CAST(ForDate AS DATE)=@Date AND CAST(LatCode AS INT)=0
	...
	SELECT CONVERT(VARCHAR,@DayStart,109) DayStart , CONVERT(VARCHAR,@DayEnd,109) DayEnd , @DayStartLatCode DayStartLatCode,@DayStartLongCode DayStartLongCode,@DayStartAddress DayStartAddress,@DayEndLatCode DayEndLatCode,@DayEndLongCode DayEndLongCode,@DayEndAddress DayEndAddress
