...
CREATE proc [dbo].[spImportRoutePlanningMstr_DSE]
@LoginId int,
@FileSetId int
as
begin
...
select distinct d.NodeID,d.NodeType,a.ApplicableStartDate INTO #DSEData from tmpRoutePlanRawData a 
join tblMstrPerson d on a.DSECode=d.Code and d.NodeType=220 
WHERE a.DSEType='SO'
union all
select distinct d.NodeID,d.NodeType,a.ApplicableStartDate  from tmpRoutePlanRawData a 
join tblMstrPerson d on a.DSECode=d.Code and d.NodeType=210 
WHERE a.DSEType='ASM'

delete a from tblRoutePlanningMstr a join #DSEData  b on a.DSENodeId=b.NodeID
...
set identity_insert tblRoutePlanningMstr  on
insert into tblRoutePlanningMstr (RouteWkPlnId, RouteNodeId, RouteNodeType, WeekNo, DayOfWeek, RouteCycId, RouteWkId,dsenodeid,dsenodetype, FromDate, ToDate, LoginIdIns, TmeStampIns, FileSetId)
select a.RouteWkPlnId,a.RouteNodeId,a.RouteNodeType,a.WeekNo,a.[DayOfWeek] ,a.RouteCycId,a.RouteWkId,a.dsenodeid,a.dsenodetype,a.FromDate,'2050-12-31',a.LoginIdIns,a.TmeStampIns,a.FileSetId from [tblRoutePlanningMstr_History] a join #DSEData  b on a.DSENodeId=b.NodeID
and a.DSENodeType=b.NodeType
where convert(date,GETDATE()) between a.FromDate and a.ToDate
set identity_insert tblRoutePlanningMstr  off


delete a from [tblRoutePlanningMstr_History] a join #DSEData  b on a.DSENodeId=b.NodeID
and a.DSENodeType=b.NodeType
where convert(date,GETDATE()) between a.FromDate and a.ToDate

insert into [tblRoutePlanningMstr_History](RouteWkPlnId, RouteNodeId, RouteNodeType, WeekNo, DayOfWeek, RouteCycId, RouteWkId,dsenodeid,dsenodetype, FromDate, ToDate, LoginIdIns, TmeStampIns, FileSetId)
select a.RouteWkPlnId,a.RouteNodeId,a.RouteNodeType,a.WeekNo,a.[DayOfWeek] ,a.RouteCycId,a.RouteWkId,a.dsenodeid,a.dsenodetype,a.FromDate,dateadd(dd,-1,b.ApplicableStartDate),a.LoginIdIns,a.TmeStampIns,a.FileSetId from tblRoutePlanningMstr a join #DSEData  b on a.DSENodeId=b.NodeID
and a.DSENodeType=b.NodeType
...
delete b from #DSEData a join tblrouteplanningvisitdetail b on a.NodeID=b.routenodeid
and a.NodeType=b.RouteNodetype
and a.ApplicableStartDate<=b.VisitDate

Declare @MinDate date
select @MinDate=Min(ApplicableStartDate) from #DSEData
...
from tblRoutePlanningMstr a join #DSEData b on a.DSENodeId=b.NodeID
and a.DSENodeType=b.NodeType
