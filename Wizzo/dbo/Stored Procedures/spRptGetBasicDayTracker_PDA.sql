...
--[spRptGetBasicDayTracker_PDA]'22-Dec-2021','E1736117-8A5C-4508-BFE9-6368F84A8663'
CREATE PROC [dbo].[spRptGetBasicDayTracker_PDA]
@strDate DATE,
@PDACOde VARCHAR(50)
AS
	...
	EXEC [spRptGetFullSalesHierarchyBasedonPDA] @PDACode,0,0
	...
	*/
	...
	--Actual Calls
	SELECT R.CovAreaId,R.CovAreaNodeType,R.ASMAreaId,R.ASMAreaNodeType,V.VisitId,V.VisitDate,V.RouteID,V.RouteType AS RouteNodeType,V.StoreID,V.BatteryLeftStatus, V.DeviceVisitStartTS, V.DeviceVisitEndTS,DATEDIFF(ss,DeviceVisitStartTS,DeviceVisitEndTS) TimeSpentInStore,ISNULL(V.EntryPersonNodeId,V.SalesPersonId) SalesmanId INTO [#VIsitedStores]
	FROM tblVisitMaster V INNER JOIN #tmpRsltWithFullHierarchy R ON V.RouteID=R.RouteID AND V.RouteType=R.RouteNodeType
	...
	*/
	...
	Select CovAreaId,CovAreaNodeType, ISNULL(SUM(OrderQty),0) AS OrderQty,SUM(OrderQtyInKG) OrderQtyInKG,SUM(OrderVal) AS TotOrderVal, COUNT(CAST(SKUNodeId AS VARCHAR)+'-'+ CAST(StoreID AS VARCHAR)) AS TotLinesOrdered,COUNT(DISTINCT StoreID) AS Prod_Call INTO #ForUpdate_CovArea  
	...
	INSERT INTO #Final(ASMAreaId,ASMAreaNodeType,[ASM Area],SOAreaId,SOAreaNodeType,[SO Area],CoverageAreaId,CoverageAreaNodeType,CoverageArea, SalesmanNodeId,SalesmanNodeType,Salesman)
	SELECT DISTINCT ASMAreaId,ASMAreaNodeType,ASMArea,SOAreaId,SOAreaNodeType,SOArea,CovAreaId,CovAreaNodeType,CovArea,SalesmanNodeId,SalesmanNodeType, Salesman
	...
	UPDATE A SET [Start Time] = CASE WHEN DATEPART(hh,Start_Time)<10 THEN '0'+ CAST(DATEPART(hh,Start_Time) AS VARCHAR)  ELSE CAST(DATEPART(hh,Start_Time) AS VARCHAR)							End+':'+ CASE WHEN DATEPART(mi,Start_Time)<10 THEN '0'+ CAST(DATEPART(mi,Start_Time) AS VARCHAR) ELSE CAST(DATEPART(mi,Start_Time)								AS VARCHAR) End,
				[End Time] = CASE WHEN DATEPART(hh,end_Time)<10 THEN '0'+ CAST(DATEPART(hh,end_Time) AS VARCHAR) ELSE CAST(DATEPART(hh,end_Time) AS VARCHAR) End+':'+
	...
	--SELECT * FROM #ForUpdate_CovArea
	UPDATE  #Final SET [Calls^Productive]=Prod_Call,[Total Sales^Dstr] = #ForUpdate_CovArea.Prod_Call,[Total Sales^Lines Ordered]=#ForUpdate_CovArea.TotLinesOrdered,[Total Sales^Qty In KG]=#ForUpdate_CovArea.OrderQtyInKG,[Total Sales^Value] = ROUND(ISNULL(#ForUpdate_CovArea.TotOrderVal,0),0), [Total Sales^Lines/Bill] =CASE ISNULL(#ForUpdate_CovArea.Prod_Call,0) WHEN 0 THEN NULL ELSE ROUND(#ForUpdate_CovArea.TotLinesOrdered/CAST(#ForUpdate_CovArea.Prod_Call AS FLOAT),2) END
	...
	BEGIN
		...
		SELECT @strSQL='UPDATE A SET A.[Order Qty In KG^' + @Category + '$2|5F8B41~80B45C]=ROUND(B.OrderQtyInKG,2) FROM #Final A INNER JOIN (SELECT CovAreaId,CovAreaNodeType,SUM(OrderQtyInKG) OrderQtyInKG FROM #TMPSales WHERE CategoryId=' + CAST(@CatNodeId AS VARCHAR) + ' GROUP BY CovAreaId,CovAreaNodeType) B ON A.CoverageAreaId=B.CovAreaId AND A.CoverageAreaNodeType=B.CovAreaNodeType'
		PRINT @strSQL
		EXEC(@strSQL)

		SELECT @StrCategory= @StrCategory + ',[Order Qty In KG^' + @Category + '$2|5F8B41~80B45C]'
		SELECT @StrCategoryForGrouping=@StrCategoryForGrouping +  ',SUM([Order Qty In KG^' + @Category + '$2|5F8B41~80B45C])'
		SELECT @Counter+=1
	END
	--SELECT * FROM #Final
	...
	BEGIN
		...
		----SELECT @strSQL='INSERT INTO #Final(flg,Lvl,flgGrouping,ASMAreaId,SOAreaId,CoverageAreaId,[Stores Added^Total],[Stores Added^Today],[Calls^Target],[Calls^Actual],[Calls^Productive],[Total Sales^Dstr],[Total Sales^Lines Ordered],[Total Sales^Qty In KG],[Total Sales^Value],[Total Sales^Lines/Bill]' + @StrCategory + ')
		...
		SELECT @strSQL='INSERT INTO #Final(flg,Lvl,flgGrouping,[ASM Area],ASMAreaId,[SO Area],SOAreaId,CoverageAreaId,[Stores Added^Total],[Stores Added^Today],[Calls^Target],[Calls^Actual],[Calls^Productive],[Total Sales^Dstr],[Total Sales^Lines Ordered],[Total Sales^Qty In KG],[Total Sales^Value],[Total Sales^Lines/Bill]' + @StrCategory + ')
		SELECT 1,3,''0,9'','''',0,''Grand Total'',0,0,SUM([Stores Added^Total]),SUM([Stores Added^Today]),SUM([Calls^Target]),SUM([Calls^Actual]),SUM([Calls^Productive]),SUM([Total Sales^Dstr]),SUM([Total Sales^Lines Ordered]),SUM([Total Sales^Qty In KG]),SUM([Total Sales^Value]),CASE ISNULL(SUM([Calls^Productive]),0) WHEN 0 THEN NULL ELSE ROUND(SUM([Total Sales^Lines Ordered])/CAST(SUM([Calls^Productive]) AS FLOAT),2) END' + @StrCategoryForGrouping + '
		FROM #Final WHERE CoverageAreaId>0'
		PRINT @strSQL
		EXEC(@strSQL)
	END
	...
	SELECT @strSQL='SELECT [SO Area] [SO Area^$1],[Day Start Time] [Day Start Time^$2],Activity [Activity^$1],[Day End Time] [Day End Time^$2],[Start Time] [First Store Visit Time^$2],[End Time] [Last Store Visit Time^$2],[Working Hours] [Working Hours (hh:mm)^$2],[Time Spent in Store] [Time Spent in Store (hh:mm)^$2],[Stores Added^Total] [Total Stores^InCoverage$2|FFD966~FFF2CC],[Stores Added^Today] [Total Stores^Today$2|FFD966~FFF2CC],[Calls^Target] [Callage^Target$2|A9D08E~E2EFDA],[Calls^Actual] [Callage^Actual$2|A9D08E~E2EFDA],[Calls^Productive] [Callage^Productive$2|A9D08E~E2EFDA],[Calls^Productivity %] [Callage^Productivity %$2|A9D08E~E2EFDA],[Total Sales^Dstr] [Total Sales^Dstr$2|A0A0A0~CDCDCD],[Total Sales^Lines Ordered] [Total Sales^Lines Ordered$2|A0A0A0~CDCDCD],ROUND([Total Sales^Qty In KG],2) [Total Sales^Qty In KG$2|A0A0A0~CDCDCD],[Total Sales^Value] [Total Sales^Value$3|A0A0A0~CDCDCD],[Total Sales^Lines/Bill] [Total Sales^Avg LPC$2|A0A0A0~CDCDCD]' + @StrCategory + ',flg [flg],flgGrouping [flgGrouping],CoverageAreaId, CoverageAreaNodeType,SalesmanNodeId,SalesmanNodeType
	FROM #Final WHERE flgHide=0 ORDER BY flg DESC,[SO Area],CoverageArea'
	PRINT @strSQL
	EXEC(@strSQL)
