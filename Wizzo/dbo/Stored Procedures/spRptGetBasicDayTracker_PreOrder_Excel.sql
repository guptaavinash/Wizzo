...
--[spRptGetBasicDayTracker_PreOrder_Excel]'31-Dec-2021',4492,''
CREATE PROC [dbo].[spRptGetBasicDayTracker_PreOrder_Excel]
@strDate DATE,
@LoginId INT,
...
AS	
	...
	*/
	...
	SELECT CovAreaId,CovAreaNodeType,SUM(TimeSpentInStore) TimeSpentInStore,CAST('' AS VARCHAR(10)) AS TimeSpentInStore_HHMM,CASE WHEN SUM(TimeSpentInStore)>=18000 THEN 1 ELSE 0 END flgWorking INTO #TimeSpentInStore 
	...
	*/
	...
	Select CovAreaId,CovAreaNodeType, ISNULL(SUM(OrderQty),0) AS OrderQty,SUM(OrderQtyInKG) OrderQtyInKG,SUM(OrderInCase) OrderInCase,SUM(OrderVal) AS TotOrderVal, COUNT(CAST(SKUNodeId AS VARCHAR)+'-'+ CAST(StoreID AS VARCHAR)) AS TotLinesOrdered,COUNT(DISTINCT StoreID) AS Prod_Call INTO #ForUpdate_CovArea  
	FROM #TMPSales GROUP BY CovAreaId,CovAreaNodeType
	--SELECT * FROM #ForUpdate_CovArea
	

	select A.PersonNodeId,A.PersonNodeType,A.[Datetime] AS DayStartTime,B.ReasonID,B.ReasonDescr INTO #DayStartDetail
	...
	UPDATE #BatteryStatus SET Start_TimeWithDate=CAST(CAST(@strDate AS VARCHAR) + ' ' + CAST(CONVERT(VARCHAR(8),Start_Time,108) AS VARCHAR) AS DATETIME)
	...
	WHERE ForDate=@strDate GROUP BY B.SalesmanNodeId
	...
	UPDATE #BatteryStatus SET WorkingHours_FirstVisitMinusDayEnd=CAST(DATEDIFF(MINUTE,Start_TimeWithDate,End_TimeWithDate)/60 AS VARCHAR) + ':' + RIGHT('0' + CAST(CAST(DATEDIFF(MINUTE,Start_TimeWithDate,End_TimeWithDate)%60 AS INT) AS VARCHAR),2)
	...
	UPDATE A SET [Start Time] = CASE WHEN DATEPART(hh,Start_Time)<10 THEN '0'+ CAST(DATEPART(hh,Start_Time) AS VARCHAR)  ELSE CAST(DATEPART(hh,Start_Time) AS VARCHAR) End+':'+ CASE WHEN DATEPART(mi,Start_Time)<10 THEN '0'+ CAST(DATEPART(mi,Start_Time) AS VARCHAR) ELSE CAST(DATEPART(mi,Start_Time) AS VARCHAR) End+' Bt@'+CAST(MIN_Battery AS VARCHAR),[End Time] = CASE WHEN DATEPART(hh,end_Time)<10 THEN '0'+ CAST(DATEPART(hh,end_Time) AS VARCHAR) ELSE CAST(DATEPART(hh,end_Time) AS VARCHAR) End+':'+
CASE WHEN DATEPART(mi,end_Time)<10 THEN '0'+ CAST(DATEPART(mi,end_Time) AS VARCHAR) ELSE CAST(DATEPART(mi,end_Time) AS VARCHAR) End+' Bt@'+CAST(MAX_Battery AS VARCHAR),[Working Hours]=B.WorkingHours
	...
	UPDATE A SET [Start Time] = CASE WHEN DATEPART(hh,Start_Time)<10 THEN '0'+ CAST(DATEPART(hh,Start_Time) AS VARCHAR)  ELSE CAST(DATEPART(hh,Start_Time) AS VARCHAR) End+':'+ CASE WHEN DATEPART(mi,Start_Time)<10 THEN '0'+ CAST(DATEPART(mi,Start_Time) AS VARCHAR) ELSE CAST(DATEPART(mi,Start_Time) AS VARCHAR) End,[End Time] = CASE WHEN DATEPART(hh,end_Time)<10 THEN '0'+ CAST(DATEPART(hh,end_Time) AS VARCHAR) ELSE CAST(DATEPART(hh,end_Time) AS VARCHAR) End+':'+
CASE WHEN DATEPART(mi,end_Time)<10 THEN '0'+ CAST(DATEPART(mi,end_Time) AS VARCHAR) ELSE CAST(DATEPART(mi,end_Time) AS VARCHAR) End,[Working Hours]=B.WorkingHours
	...
	UPDATE F SET [Calls^Avg Retailing Time(Sec)]=T.TimeSpentInStore/[Calls^Actual],[Calls^Retailing Grade]=CASE flgWorking WHEN 1 THEN 'Greater Than 5 Hours' ELSE 'Less Than 5 Hours' END FROM #Final F INNER JOIN #TimeSpentInStore T ON T.CovAreaId=F.CoverageAreaId AND T.CovAreaNodeType=F.CoverageAreaNodeType
	...
	--SELECT * FROM #ForUpdate_CovArea
	UPDATE  #Final SET [Calls^Productive]=Prod_Call,[Total Sales^Dstr] = #ForUpdate_CovArea.Prod_Call,[Total Sales^Lines Ordered]=#ForUpdate_CovArea.TotLinesOrdered,[Total Sales^Qty In KG]=#ForUpdate_CovArea.OrderQtyInKG,[Total Sales^Qty In Case]=#ForUpdate_CovArea.OrderInCase,[Total Sales^Value] = ROUND(ISNULL(#ForUpdate_CovArea.TotOrderVal,0),0), [Lines/Bill] =CASE ISNULL(#ForUpdate_CovArea.Prod_Call,0) WHEN 0 THEN NULL ELSE ROUND(#ForUpdate_CovArea.TotLinesOrdered/CAST(#ForUpdate_CovArea.Prod_Call AS FLOAT),2) END
	...
	SELECT @strSQL='SELECT [RSM Area] ,[ASM Area] ,[SO Area] ,DOJ [Date Of Joining],[Day Start Time] ,Activity ,[Day End Time] ,[Start Time] ,[End Time] ,WorkingHours_FirstVisitMinusDayEnd [Working Hours] ,[Calls^Avg Retailing Time(min:Sec)],[Calls^Retailing Grade],
	[Time Spent in Store] ,[Stores Added^Total] [Stores InCoverage] ,[Stores Added^Today] [Stores Added TOday] ,[Calls^Target] [Target Calls],[Calls^Actual] [Visits Made],[Calls^Productive] [Productive Calls],[Calls^Productivity %][Productivity %],[Total Sales^Dstr] [Sales Distribution],[Total Sales^Lines Ordered][Lines Ordered],ROUND([Total Sales^Qty In KG],2) [Qty In KG],ROUND([Total Sales^Qty In Case],2) [Qty In Case],[Total Sales^Value] [Sales Value],[Lines/Bill] ' + @StrCategory + ',flg [flg],flgGrouping [flgGrouping] 
