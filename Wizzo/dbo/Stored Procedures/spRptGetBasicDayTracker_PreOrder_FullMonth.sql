...
--[spRptGetBasicDayTracker_PreOrder_FullMonth]'31-Mar-2022',4492,''
CREATE PROC [dbo].[spRptGetBasicDayTracker_PreOrder_FullMonth]
@strDate DATE,
@LoginId INT,
...
AS	
	...
	*/
	...
	UPDATE F SET ASM=V.ASM FROM #Final F INNER JOIN VwCompanyDSRFullDetail V ON V.ASMAreaID=F.ASMAreaId AND V.ASMAreaNodeType=F.ASMAreaNodeType 
	...
	UPDATE F SET ReasonID=D.ReasonID,Activity=D.ReasonDescr FROM #Final F INNER JOIN #DayStartDetail D ON D.PersonNodeID=F.SalesmanNodeId AND CAST(D.AttenDate AS DATE)=F.RptDate
	...
	SELECT @strSQL='SELECT RptDate,[RSM Area],RSMName ,[ASM Area],ASM ,[SO Area],Salesman ,AssignRoute,VisitRoute,CASE WHEN AssignRoute=VisitRoute THEN ''Yes'' ELSE ''No'' END AS [PJP Adherence],FORMAT(CAST(DOJ AS DATE),''dd-MMM-yy'') [Date Of Joining],[Start Time] ,[End Time],Activity ,[Working Hours] [Working Hours] ,[Calls^Avg Retailing Time(hour:min:Sec)] [Avg Retailing Time(hour:min:Sec)],[Calls^Retailing Grade] [Retailing Grade],[Time Spent in Store] ,[Calls^Target] [Schedule Calls],[Calls^Actual] [Total Calls],[Calls^Productive] [Productive Shops],[Calls^Productivity %][Productivity %]' + @StrCategory + ',[Total Sales^Value] [Sales Value]
