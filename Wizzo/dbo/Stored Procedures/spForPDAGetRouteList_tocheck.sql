...
-- EXEC spForPDAGetRouteList 'C0694CFC-F5BE-48E1-AC94-AF8CC4A419CE','30-Nov-2021',1
CREATE PROCEDURE [dbo].[spForPDAGetRouteList_tocheck]  
@PDACode VARCHAR(50),  
@strDate VARCHAR(20), 
@AppVersionID INT--,
...
AS  
	...
	BEGIN
		...
		INSERT INTO #Routes(Nodeid,Nodetype,Fromdate,ToDate)
		SELECT DISTINCT  RouteNodeId,RouteNodetype,GETDATE(),'31-Dec-2049' FROM tblRoutePlanningVisitDetail WHERE VisitDate>=CAST(GETDATE() AS DATE) AND DSENodeId=@PersonID AND DSENodeType=@PersonType
	...
	SELECT DISTINCT A.NodeId ,A.NodeType,ISNULL(A.RouteCode,'') + '-' + A.Descr 
	FROM [dbo].[tblCompanySalesStructureRouteMstr] A 
	INNER JOIN #Routes C on C.NodeId=A.NodeId AND C.NodeType=A.NodeType
	

	UPDATE A SET Active=1 FROM #AllRoutes A INNER JOIN tblRoutePlanningVisitDetail R ON R.RouteNodeID=A.DSRRouteID AND R.RouteNodetype=A.DSRRouteNodeType AND CAST(VisitDate AS DATE)=CAST(@strDate AS DATE)
