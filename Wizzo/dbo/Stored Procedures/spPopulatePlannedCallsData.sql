...
CREATE proc [dbo].[spPopulatePlannedCallsData] 
@FileSetId int
as
begin
return;
Declare @currDate datetime=dbo.fnGetCurrentDateTime()
select r.nodeid as RouteNodeId,r.nodetype as RouteNodeType,p.nodeid as DSENodeId,p.NodeType as DSENodeType,s.StoreId,b.nodeid as DistNodeId,b.nodetype as DistNodeType,1 as SectorId,convert(numeric(18,2),0) as Total,convert(float,0) as #OfInv,'Planned Calls' as Reason,r.Descr as  RouteName,p.Descr as DSEName,ClusterType  into #StoresList
 from mrco_Console2Minet_TCOutletSchedule_Astic a join tblDBRSalesStructureDBR b on a.DistCode=b.DistributorCode
join tblStoreMaster s on s.StoreCode=a.ChildRtrCode 
...
where Calldate=convert(date,@currDate)
and b.FlgLive=1 and a.CycFileID=@FileSetId
...
,NoOfPendingVisits,RouteName,DSEName,LanguageId,RouteGTMType,LotId,ClusterType)

select 0,a.StoreId,convert(date,getdate()),a.DSENodeId,a.DSENodeType,a.RouteNodeId,a.RouteNodeType,a.DistNodeId,a.DistNodeType,b.storecode,b.storename,isnull(b.contactperson,''),isnull(b.contactno,'')+isnull(','+b.MobileNo1,'')+isnull(','+b.MobileNo2,''),ch.ChannelName,0,null,0,row_number() over(order by case when [#OfInv]>0 then Total/[#OfInv] else 0 end desc),Total,[#OfInv],a.ReasonId,0,0,0,a.SectorId,b.OutStandingAmt,b.OutStandingDate,b.ChannelId,0,3,
...
,a.NoOfPendingVisits,a.RouteName,A.DSEName,b.LanguageId,b.RouteGTMType,b.LotId,a.ClusterType
 from 
...
left join tblChannelWiseProductivityThreshold cw on cw.ChannelId=b.ChannelId and convert(date,@currDate) between cw.FromDate and cw.ToDate
left join tblTeleCallerListForDay z on a.StoreId=z.StoreId and z.Date=convert(date,@currDate)
