...
--select * from tbltelecallerlistforday orde
--exec spSchemeDetailByStoreN @storeid=919223,@Date='2020-04-17 00:00:00'
--[spSchemeDetailByStore]    356129,'27-jan-2021'
CREATE proc [dbo].[spSchemeDetailByStore_20210128]   
@Storeid int,  
@Date datetime  
AS  
begin  
...
Declare @ChannelId int,@SubChannelId int,@DistributorNodeId int,@DistributorNodeType int,@BranchSubdNodeId int,@BranchNodeType int
...
@DistributorNodeType=DistNodeType from tblStoreMaster s   where StoreID=@Storeid
...
select a.PrdNodeId,a.PrdNodeType into #PrdBranchSBFPriceMstr from tblPrdDistributorProductPriceMstr(nolock) a 
join 
tblPrdactivePRDList (nolock) z on a.PrdNodeId=z.PrdNodeId and a.PrdNodeType=z.PrdNodeType
and a.DistNodeId=z.DistNodeId
and a.DistNodeType=z.DistNodeType
where a.DistNodeId=@DistributorNodeId and a.DistNodeType=@DistributorNodeType 
...
select Distinct B.RowID,0 AS ProductID,'' AS FreeProductName,'' AS SKUCode,5 AS UOMID,0 as StandardRate,0 AS StandardRateBeforeTax ,0 AS Tax,0 AS MRP,0 as Grammage,0 as CurrentInvStock,0 as BookedInvStock from #BenSplit A   
join #tmpBenBucketDetails B ON A.SchemeSlabID=B.SchemeSlabID  
AND A.BucketID=B.BucketID AND A.SubBucketID=B.SubBucketID  
  where 1<>1
