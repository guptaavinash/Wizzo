...
-- =============================================
--EXEC [spMDXQuery_SummaryDataForASM]'DFE2E396-DA11-47B9-B19D-7C603379F038','31-Mar-2022',0,0
CREATE Procedure [dbo].[spMDXQuery_SummaryDataForASM_Grv]
@IMEINo VARCHAR(50),
@RptDate DATE,
...
BEGIN
	...
	CREATE TABLE #Past4WeekData_StoreLvl(ASMArea VARCHAR(200),BrandId VARCHAR(20),StoreId VARCHAR(20),ProdVisit INT,NewStoreDistributionVisit INT,Ordr INT)
	CREATE TABLE #BeforePast4WeekData_StoreLvl(ASMArea VARCHAR(200),BrandId VARCHAR(20),StoreId VARCHAR(20),ProdVisit INT,NewStoreDistributionVisit INT,Ordr INT)
	...
	BEGIN
		...
		*/
		...
		TRUNCATE TABLE #TmpTime
		INSERT INTO #TmpTime(TimeVal,NodeType)
		SELECT TOP 28 CONVERT(VARCHAR,[Date],112),1 FROM tblOlapTimeHierarchy_Day WHERE [Date]<=@YesterdayDate ORDER BY [Date] DESC
		SELECT * FROM #TmpTime
		...
		SET @strMDX='SELECT {[Measures].[EffectiveVisitCount_ActiveCoverage],[Measures].[NewStoresOrderedDistribution_ActiveCoverage]} ON COLUMNS, NONEMPTY  ({' + @GroupStr + ' }) DIMENSION PROPERTIES MEMBER_CAPTION ON ROWS '
		...
		BEGIN TRY  
			IF @strMDXTime<>''
			BEGIN
				INSERT INTO #Past4WeekData_StoreLvl(ASMArea,StoreId,BrandId,ProdVisit,NewStoreDistributionVisit)
				EXECUTE SP_EXECUTESQL @OPEN_QUERY 
			END
		END TRY  
		BEGIN CATCH  
		   --SELECT ERROR_NUMBER() AS ErrorNumber   
		END CATCH 
		DELETE FROM #Past4WeekData_StoreLvl WHERE ISNULL(NewStoreDistributionVisit,0)=0
		UPDATE #Past4WeekData_StoreLvl SET BrandId=0,Ordr=2 WHERE BrandId IS NULL
		SELECT * FROM #Past4WeekData_StoreLvl WHERE ISNULL(NewStoreDistributionVisit,0)>0 AND ProdVisit>=2 order by StoreId,BrandId
		SELECT * FROM #Past4WeekData_StoreLvl WHERE ISNULL(NewStoreDistributionVisit,0)>0 AND BrandId=0 and ProdVisit>=2 order by StoreId,BrandId
		SELECT * FROM #Past4WeekData_StoreLvl WHERE ISNULL(NewStoreDistributionVisit,0)>0 AND BrandId<>0 and ProdVisit>=2 order by BrandId,StoreId


		PRINT 'Before past 4 Weeks'
		TRUNCATE TABLE #TmpTime
		--INSERT INTO #TmpTime(TimeVal,NodeType)
		SELECT CONVERT(VARCHAR,[Date],112),1 FROM tblOlapTimeHierarchy_Day WHERE [Date]<DATEADD(dd,-27,@YesterdayDate) ORDER BY [Date] DESC
		SELECT * FROM #TmpTime
	...
	*/
	...
	BEGIN		
		...
		UPDATE A SET A.New_Dstrbn=B.ProdStores FROM #Distribution A, (SELECT COUNT(DISTINCT StoreId) ProdStores FROM #Past4WeekData_StoreLvl WHERE BrandId=0 AND ISNULL(NewStoreDistributionVisit,0)>0) B WHERE A.BrandId=0

		UPDATE A SET A.New_Dstrbn=B.ProdStores FROM #Distribution A INNER JOIN (SELECT BrandId,COUNT(DISTINCT StoreId) ProdStores FROM #Past4WeekData_StoreLvl WHERE BrandId<>0 AND ISNULL(NewStoreDistributionVisit,0)>0 GROUP BY BrandId) B ON A.BrandId=B.BrandId
		...
		UPDATE A SET A.New_Dstrbn=B.ProdStores FROM #Distribution2X A, (SELECT COUNT(DISTINCT StoreId) ProdStores FROM #Past4WeekData_StoreLvl WHERE BrandId=0 AND ProdVisit>=2 AND ISNULL(NewStoreDistributionVisit,0)>0) B WHERE A.BrandId=0

		UPDATE A SET A.New_Dstrbn=B.ProdStores FROM #Distribution2X A INNER JOIN (SELECT BrandId,COUNT(DISTINCT StoreId) ProdStores FROM #Past4WeekData_StoreLvl WHERE BrandId<>0 AND ProdVisit>=2 AND ISNULL(NewStoreDistributionVisit,0)>0 GROUP BY BrandId) B ON A.BrandId=B.BrandId
