...
--EXEC [spRptPerfomanceOverView] '20-AUG-2021','20-AUG-2021','','',95,1,0
CREATE procedure [dbo].[spRptPerfomanceOverView]
@FromDate date,
@ToDate date,
...
Create nonclustered index IDX_BranchList ON #ASoList(SOAreaNodeId,SOAreaNodeType) include(Zone ,Region ,ASMArea ,SOArea  
...
print CONVERT(varchar, dbo.fnGetCurrentDateTime(),108)
		...
		join #Sector sc on sc.SectorId=c.SectorId
...
CREATE TABLE #TeleData(SONodeId INT,SONodeType INT,flgorder INT,DistNodeId INT,DistNodeType INT, [Tele Caller] VARCHAR(1000),[# Calls Planned] INT,[# Phone Available] INT, [# Calls Made] INT, [# Calls Picked] INT, [# Failed Calls] INT,[# Unproductive Calls] INT, [# Calls Productive] INT, [# Lines Ordered] INT, [Value Ordered] DECIMAL(18,2),[Order Golden Points Added] int
...
[# of Star Points] int,[# of 5 Star Calls] int, [Pending Supplied] decimal(18,2)
)
...
Create nonclustered index IDX_ActOrderData ON #ActOrderVal(TeleCallingId) include(OrderVal)
		...
		select a.SONodeId,a.SONodeType, 0,a.DistNodeId,a.DistNodeType, b.TeleCallerCode as  [Tele Caller],Count(*) as [# Calls Planned],
		sum(convert(int,IsValidContactNo)) as [# Phone Available],
		...
		group by a.TeleCallingId,b.TeleCallerCode,a.DistNodeId,a.DistNodeType
		,a.TCNodeId,a.Date,a.StoreId,a.ChannelId,a.SONodeId,a.SONodeType
...
union all
select '# of Distributor'AS Txt,convert(varchar,Count(distinct DistNodeId)) as Value,'d5fca0' as Color from   
#TeleData b --WHERE EXISTS(SELECT Count() FROM #TeleData WHERE DistNodeId=B.DistNodeId)
...
Create table #FinalReport(Lvl tinyint,NodeId int,NodeType int,PNodeId int,PNodeType int, [Total\Zone\Region\ASM\SO\DB] varchar(500),
[# Of TES^1] int,
...
,
case when COUNT(Distinct  case when [Tele Caller] is not null then  convert(varchar,TeleUserId)+'-'+convert(varchar,Date,106) end)>0 then 
(sum([Value Ordered]))/COUNT(Distinct  case when [Tele Caller] is not null then  convert(varchar,TeleUserId)+'-'+convert(varchar,Date,106) end) else 0 end)
...
(case when COUNT(Distinct  case when [Tele Caller] is not null then  convert(varchar,TeleUserId)+'-'+convert(varchar,Date,106) end)>0 then 
(sum([Value Supplied]))/COUNT(Distinct  case when [Tele Caller] is not null then  convert(varchar,TeleUserId)+'-'+convert(varchar,Date,106) end) else 0 end) as [Throughput Per TES],
...
print CONVERT(varchar, dbo.fnGetCurrentDateTime(),108)
insert into #FinalReport
select 2,b.ZoneNodeId,b.ZoneNodeType,0,0,b.Zone,COUNT(Distinct case when [Tele Caller] is not null then  convert(varchar,TeleUserId)+'-'+convert(varchar,Date,106) end),sum([# Calls Planned]),sum([# Phone Available]),sum([# Calls Made]),sum([# Calls Picked]),sum([# Calls Productive])
,sum([# Lines Ordered]),
sum([Value Ordered])
,convert(numeric(20,2),
case when sum([# Calls Productive])>0 then 
...
case when sum([Value Supplied])>0 then (sum([Return Value])*100)/sum([Value Supplied]) else 0 end)) [SRN%]
 ...
 from #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType group by b.ZoneNodeId,b.ZoneNodeType,b.Zone
 ORDER BY Zone

 UPDATE A SET A.[# Stores Delivered^2]=B.StoresServed FROM #FinalReport A INNER JOIN (SELECT ZoneNodeId,ZoneNodeType,COUNT(DISTINCT StoreIdServed) StoresServed FROM #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType WHERE StoreIdServed>0 GROUP BY ZoneNodeId,ZoneNodeType) B ON A.NodeId=B.ZoneNodeId AND A.NodeType=B.ZoneNodeType
 
 
 UPDATE A SET A.[# Stores Pending Dlvry^2]=B.StoresServed  FROM #FinalReport A INNER JOIN (SELECT ZoneNodeId,ZoneNodeType,COUNT(DISTINCT StoreId) StoresServed FROM #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType WHERE flgSupplyPending>0 GROUP BY ZoneNodeId,ZoneNodeType) B ON A.NodeId=B.ZoneNodeId AND A.NodeType=B.ZoneNodeType
 ...
 ----------------------------------RSH
 insert into #FinalReport
select 3,b.RegionNodeId,b.RegionNodeType,b.ZoneNodeId,b.ZoneNodeType,b.Region,COUNT(Distinct case when [Tele Caller] is not null then  convert(varchar,TeleUserId)+'-'+convert(varchar,Date,106) end),sum([# Calls Planned]),sum([# Phone Available]),sum([# Calls Made]),sum([# Calls Picked]),sum([# Calls Productive])
,sum([# Lines Ordered]),
sum([Value Ordered])
,convert(numeric(20,2),
case when sum([# Calls Productive])>0 then 
...
case when sum([Value Supplied])>0 then (sum([Return Value])*100)/sum([Value Supplied]) else 0 end)) [SRN%]
 ...
 from #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType group by b.ZoneNodeId,b.ZoneNodeType,b.RegionNodeId,b.RegionNodeType,b.Region
 ORDER BY Region

 UPDATE A SET A.[# Stores Delivered^2]=B.StoresServed FROM #FinalReport A INNER JOIN (SELECT RegionNodeId,RegionNodeType,COUNT(DISTINCT StoreIdServed) StoresServed FROM #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType WHERE StoreIdServed>0 GROUP BY RegionNodeId,RegionNodeType) B ON A.NodeId=B.RegionNodeId AND A.NodeType=B.RegionNodeType
 
 
 UPDATE A SET A.[# Stores Pending Dlvry^2]=B.StoresServed  FROM #FinalReport A INNER JOIN (SELECT RegionNodeId,RegionNodeType,COUNT(DISTINCT StoreId) StoresServed FROM #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType WHERE flgSupplyPending>0 GROUP BY RegionNodeId,RegionNodeType) B ON A.NodeId=B.RegionNodeId AND A.NodeType=B.RegionNodeType
 ...
 ----------------------------------RSH
 insert into #FinalReport
select 4,b.ASMAreaNodeId,b.ASMAreaNodeType,b.RegionNodeId,b.RegionNodeType,b.ASMArea,COUNT(Distinct case when [Tele Caller] is not null then  convert(varchar,TeleUserId)+'-'+convert(varchar,Date,106) end),sum([# Calls Planned]),sum([# Phone Available]),sum([# Calls Made]),sum([# Calls Picked]),sum([# Calls Productive])
,sum([# Lines Ordered]),
sum([Value Ordered])
,convert(numeric(20,2),
case when sum([# Calls Productive])>0 then 
...
case when sum([Value Supplied])>0 then (sum([Return Value])*100)/sum([Value Supplied]) else 0 end)) [SRN%]
 ...
 from #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType group by b.ASMAreaNodeId,b.ASMAreaNodeType,b.RegionNodeId,b.RegionNodeType,b.ASMArea
 ORDER BY ASMArea

 UPDATE A SET A.[# Stores Delivered^2]=B.StoresServed FROM #FinalReport A INNER JOIN (SELECT ASMAreaNodeId,ASMAreaNodeType,COUNT(DISTINCT StoreIdServed) StoresServed FROM #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType WHERE StoreIdServed>0 GROUP BY ASMAreaNodeId,ASMAreaNodeType) B ON A.NodeId=B.ASMAreaNodeId AND A.NodeType=B.ASMAreaNodeType
 
 
 UPDATE A SET A.[# Stores Pending Dlvry^2]=B.StoresServed  FROM #FinalReport A INNER JOIN (SELECT ASMAreaNodeId,ASMAreaNodeType,COUNT(DISTINCT StoreId) StoresServed FROM #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType WHERE flgSupplyPending>0 GROUP BY ASMAreaNodeId,ASMAreaNodeType) B ON A.NodeId=B.ASMAreaNodeId AND A.NodeType=B.ASMAreaNodeType
 ...
 ----------------------------------SO
 insert into #FinalReport
select 5,b.SOAreaNodeId,b.SOAreaNodeType,b.ASMAreaNodeId,b.ASMAreaNodeType,b.SoArea,COUNT(Distinct case when [Tele Caller] is not null then  convert(varchar,TeleUserId)+'-'+convert(varchar,Date,106) end),sum([# Calls Planned]),sum([# Phone Available]),sum([# Calls Made]),sum([# Calls Picked]),sum([# Calls Productive])
,sum([# Lines Ordered]),
sum([Value Ordered])
,convert(numeric(20,2),
case when sum([# Calls Productive])>0 then 
...
case when sum([Value Supplied])>0 then (sum([Return Value])*100)/sum([Value Supplied]) else 0 end)) [SRN%]
 ...
 from #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType group by b.SoAreaNodeId,b.SoAreaNodeType,b.ASMAreaNodeId,b.ASMAreaNodeType,b.SoArea
 ORDER BY SoArea

 UPDATE A SET A.[# Stores Delivered^2]=B.StoresServed FROM #FinalReport A INNER JOIN (SELECT SoAreaNodeId,SoAreaNodeType,COUNT(DISTINCT StoreIdServed) StoresServed FROM #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType WHERE StoreIdServed>0 GROUP BY SoAreaNodeId,SoAreaNodeType) B ON A.NodeId=B.SoAreaNodeId AND A.NodeType=B.SoAreaNodeType
 
 
 UPDATE A SET A.[# Stores Pending Dlvry^2]=B.StoresServed  FROM #FinalReport A INNER JOIN (SELECT SoAreaNodeId,SoAreaNodeType,COUNT(DISTINCT StoreId) StoresServed FROM #TeleData a join #ASoList b on a.SONodeId=b.SONodeId
 and a.SONodeType=b.SONodeType WHERE flgSupplyPending>0 GROUP BY SoAreaNodeId,SoAreaNodeType) B ON A.NodeId=B.SoAreaNodeId AND A.NodeType=B.SoAreaNodeType
 ...
 ----------------------------------Distributor
 insert into #FinalReport
select 6,d.NodeID,d.NodeType,b.SOAreaNodeId,b.SOAreaNodeType,d.Descr+' ['+d.DistributorCode+']',COUNT(Distinct case when [Tele Caller] is not null then  convert(varchar,TeleUserId)+'-'+convert(varchar,Date,106) end),sum([# Calls Planned]),sum([# Phone Available]),sum([# Calls Made]),sum([# Calls Picked]),sum([# Calls Productive])
,sum([# Lines Ordered]),
sum([Value Ordered])
,convert(numeric(20,2),
case when sum([# Calls Productive])>0 then 
...
case when sum([Value Supplied])>0 then (sum([Return Value])*100)/sum([Value Supplied]) else 0 end)) [SRN%]
 ...
 and d.NodeType=a.DistNodeType
  group by d.nodeid,d.nodetype,b.SOAreaNodeId,b.SOAreaNodeType,d.Descr,d.DistributorCode
 ORDER BY d.Descr
...
convert(float,[Net Lines Supplied^2])/[# Stores Delivered^2] else 0 end
 ...
 print '12'
print CONVERT(varchar, dbo.fnGetCurrentDateTime(),108)
 SELECT * FROM #FinalReport order by lvl,[Total\Zone\Region\ASM\SO\DB]
