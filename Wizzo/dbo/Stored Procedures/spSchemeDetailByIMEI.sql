...
CREATE proc [dbo].[spSchemeDetailByIMEI]   
@PDACode varchar(100),  
@Date datetime,
...
END
	SELECT DISTINCT C.CoverageAreaNodeID,C.CoverageAreaNodeType,@PersonNodeID PersonNodeID,@PersonNodeType PersonNodeType,RouteID,RouteNodeType,RM.Descr,RC.StoreId ,SM.ChannelId,0 SubChannelID,SM.DBID,SM.DBNodeType,0 StateId INTO #tmpRouteStoreId FROM tblRouteCoverageStoreMapping(nolock) RC INNER JOIN tblCompanySalesStructureRouteMstr(nolock) RM ON RM.NodeID=RC.RouteID AND RC.RouteNodeType=RM.NodeType
...
and ((A.DBID=z.DistributorNodeId and A.DBNodeType=z.DistributorNodeType) or Z.DistributorNodeId=0)
and (A.stateid=z.stateid or Z.stateid=0)
where @Date between b.SchemeFromDate and b.SchemeToDate
