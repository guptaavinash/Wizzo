...
CREATE PROCEDURE [dbo].[spRptGetStoreWiseDaySummary]  
@PDACode VARCHAR(50)='',  
@Date varchar(20),
...
--CREATE TABLE #tmpProduct([Sr.] INT IDENTITY(1,1),ProductId INT, Product VARCHAR(200),MRP FLOAT, Rate FLOAT,[Stock Qty] INT, [Order Qty] INT,[Free Qty] INT, [Disc Value] FLOAT,	[Net Value] FLOAT, [Tax Value] FLOAT, [Gross Value] FLOAT)	
	...
	MERGE #tmp tmpDet USING (SELECT VM.StoreId,SM.StoreName,SUM(VS.Qty*Vw.StandardRate) StockValue
	FROM            tblVisitStock VS INNER JOIN
					tblVisitMaster VM ON VS.VisitID = VM.VisitID INNER JOIN
					VwSFAProductHierarchy Vw  ON VS.ProductId = Vw.SKUNodeID INNER JOIN
					tblStoreMaster SM ON VM.StoreID = SM.StoreID --INNER JOIN #Routes ON VM.RouteID = #Routes.RouteID 
					INNER JOIN #PersonList P ON VM.SalesPersonId=P.PersonNodeId AND VM.SalesPersonType=P.PersonNodeType
	WHERE       CAST(VM.VisitDate AS DATE)=@VisitDate
	GROUP BY VM.StoreId,SM.StoreName) tmpStock
	ON tmpDet.StoreID = tmpStock.StoreID
	WHEN MATCHED THEN    
	  UPDATE    
	  SET tmpDet.[Stock Value] = tmpStock.StockValue
	WHEN NOT MATCHED BY TARGET THEN   
	  INSERT (StoreId,Store,[Stock Value])  
	  VALUES (tmpStock.StoreID,tmpStock.StoreName,tmpStock.StockValue);
	...
	SELECT Store,[Lines per Bill] as [LinesperBill],ROUND([Stock Value],0) [StockValue],ROUND([Disc Value],0) [DiscValue],ROUND(ValBeforeTax,0) ValBeforeTax,ROUND([Tax Value],0) [TaxValue],ROUND(ValAfterTax,0) ValAfterTax ,Lvl
