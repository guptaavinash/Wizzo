...
--exec [spGetOrderProductList] 40590,339943
CREATE proc [dbo].[spGetOrderProductList] 
@StoreId int,
@TeleCallingId int
as
begin
...
set @StateId=0
select * INTO #SBFPrice from [dbo].tblprdskusalesmapping(nolock) where PrcLocationId=@PrcRegionId and GETDATE() between FromDate and ToDate and UOMID=3
...
IF OBJECT_ID('tempdb..#TmpCrossTab') is not null
begin
...
SELECT DISTINCT IDENTITY(INT,1,1) AS IDENT, DisplayDate, InvDate into #Col FROM #PrdList WHERE InvDate IN(SELECT InvDate FROM #InvDT) ORDER BY InvDate DESC
...
DECLARE @strSQL varChar(2000)
	...
	SET @curDate=CURSOR FOR SELECT  DisplayDate, InvDate FROM #Col ORDER BY InvDate DESC
	OPEN @curDate
	...
	SELECT prdnodeid,prdnodetype,Qty,0 AS flgSaleType,0 as PcsInBox into #P4M FROM [tblP4MP6MStoreSuggestedData] where storeid=@StoreId
	UPDATE A SET flgSaleType=B.flgSaleType,PcsInBox=b.PcsInBox FROM #PrdList B JOIN #P4M A on 
	a.Prdnodeid=b.PrdNodeId
	IF (SELECT COUNT(*) FROM #P4M)>0
	BEGIN
	SET @i=@i+1
	SET @strSQL='ALTER TABLE #TmpCrossTab Add [PP3M] varchar(30)'
			EXEC (@strSQL)
			SET @strSQl=''
			SET @strSQl='UPDATE #TmpCrossTab SET #TmpCrossTab.[PP3M] = CASE WHEN #P4M.[flgSaleType]=2 then #P4M.Qty/#P4M.PCSInBox
			else #P4M.Qty end FROM  #TmpCrossTab INNER JOIN #P4M ON #TmpCrossTab.PrdNodeId= #P4M.PrdNodeId'
			PRINT @strSQL
			EXEC (@strSQL)
			INSERT INTO #Col VALUES('PP3M','1900-01-01')
	END
	...
	begin
				...
				EXEC (@strSQL)
				INSERT INTO #Col VALUES('NA'+convert(varchar,@i-@ik+1),'1900-01-01')
	set @i=@i+1
	end
...
A.ContactPerson,
a.ContactNo,
A.Channel,a.SOName+isnull(' ['+a.SOArea+']','') as PersonName,b.Descr+' ['+b.DistributorCode+']' as Distributor,
...
and sh.sonodetype=a.sonodetype
 ...
 join tblTeleCallerMstr tc on tc.TeleCallerId=a.TCNodeId
...
Update tblTeleCallerListForDay set CallStartDate=isnull(CallStartDate,Getdate()) where TeleCallingId=@TeleCallingId  
