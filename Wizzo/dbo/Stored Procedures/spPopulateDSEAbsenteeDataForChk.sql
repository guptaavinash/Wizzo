...
CREATE proc [dbo].[spPopulateDSEAbsenteeDataForChk] 
@FileSetId bigint
as
begin
...
select a.DivisionHeadCode,a.RSHCode,a.ASMCode,a.TSOTSECode,a.DistributorCode,a.USERID,a.UserName,a.DistUserUniqueID,a.BeatID,a.BeatName,a.DownloadFlag,a.SchedulerCycleDateTime,a.CycFileID,19,@currdate from mrco_DSRAbsentee_Data a left join tblDBRSalesStructureDBR d on a.DistributorCode=d.DistributorCode and d.flgLive=1
...
Update a set Absent=case when b.DownloadFlag='N' then 1 ELSE 0 END,FileSetIdUpd=@FileSetId,TimeStampUpd=@currdate from tblAttendanceDetailForChk a join #tempDSEData b on a.DSENodeId=b.DSENodeId
and a.DSENodeType=b.DSENodeType 
where a.AttendDate=convert(date,@currdate)  

--Update a set Absent=0,FileSetIdUpd=@FileSetId,TimeStampUpd=@currdate from tblAttendanceDetailForChk a  join #tempDSEData b on a.DSENodeId=b.DSENodeId
--and a.DSENodeType=b.DSENodeType 
--where a.AttendDate=convert(date,@currdate) and b.DSENodeId is null

Insert   into tblAttendanceDetailForChk(AttendDate,DistNodeId,DistNodeType,DSENodeId,DSENodeType,RouteNodeId,RouteNodetype,Absent,FileSetIdIns,TimeStampIns)

select distinct @currdate,b.DistNodeId,b.DistNodeType,b.DSENodeId,b.DSENodeType,0,0,case when b.DownloadFlag='N' then 1 ELSE 0 END,@FileSetId,@currdate from tblAttendanceDetailForChk a right join #tempDSEData b on a.DSENodeId=b.DSENodeId
and a.DSENodeType=b.DSENodeType 
and a.AttendDate=convert(date,@currdate) 
where a.DSENodeId is null


delete b from tblAttendanceDetailForChk a join tblTeleCallerListForDayForChk b on a.AttendDetId=b.AttendDetId
where Absent=0 and a.AttendDate=convert(date,@currdate) and b.IsUsed=0 

Declare @TeleCallingId bigint=0
Select @TeleCallingId=isnull(Max(TeleCallingId),1) from tblTeleCallerListForDayForChk

 DBCC CHECKIDENT ('tblTeleCallerListForDayForChk', RESEED, @TeleCallingId); 
...
Update a set RouteNodeId=b.RouteNodeId,RouteNodetype=b.RouteNodeType from tblAttendanceDetailForChk a join #StoresList b on a.DSENodeId=b.DSENodeId
and a.DSENodeType=b.DSENodeType
where a.AttendDate=convert(date,@currdate)


Update b set AttendDetId=a.AttendDetId from tblAttendanceDetailForChk a join #StoresList b on a.DSENodeId=b.DSENodeId
and a.DSENodeType=b.DSENodeType
where a.AttendDate=convert(date,@currdate)
...
--join tblMstrPerson p on p.NodeID=a.DSENodeId
 ...
 --select * from #StoresList
insert into tblTeleCallerListForDayForChk(AttendDetId,StoreId,Date,DSENodeId,DSENodetype,RouteNodeId,RouteNodeType,DistNodeId,DistNodeType,
...
left join tblTeleCallerListForDayForChk z on a.StoreId=z.StoreId and z.Date=convert(date,getdate())
