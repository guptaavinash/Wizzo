CREATE TYPE [dbo].[udt_RawDataCustomerVisit] AS TABLE (
    [IMEINumber]                            NVARCHAR (500) NULL,
    [StoreID]                               NVARCHAR (500) NULL,
    [StoreVisitCode]                        NVARCHAR (500) NULL,
    [VisitID]                               NVARCHAR (500) NULL,
    [VisitDate]                             NVARCHAR (500) NULL,
    [VisitLatitude]                         NVARCHAR (500) NULL,
    [VisitLongitude]                        NVARCHAR (500) NULL,
    [VisitTimeOutSideStore]                 NVARCHAR (500) NULL,
    [VisitTimeInSideStore]                  NVARCHAR (500) NULL,
    [VisitTimeCheckStore]                   NVARCHAR (500) NULL,
    [VisitEndTS]                            NVARCHAR (500) NULL,
    [LocProvider]                           NVARCHAR (500) NULL,
    [Accuracy]                              NVARCHAR (500) NULL,
    [BateryLeftStatus]                      NVARCHAR (500) NULL,
    [StoreClose]                            NVARCHAR (500) NULL,
    [AllProviderData]                       NVARCHAR (500) NULL,
    [GPSLatitude]                           NVARCHAR (500) NULL,
    [GPSLongitude]                          NVARCHAR (500) NULL,
    [GPSAccuracy]                           NVARCHAR (500) NULL,
    [GPSAddress]                            NVARCHAR (500) NULL,
    [NetworkLatitude]                       NVARCHAR (500) NULL,
    [NetworkLongitude]                      NVARCHAR (500) NULL,
    [NetworkAccuracy]                       NVARCHAR (500) NULL,
    [NetworkAddress]                        NVARCHAR (500) NULL,
    [FusedLatitude]                         NVARCHAR (500) NULL,
    [FusedLongitude]                        NVARCHAR (500) NULL,
    [FusedAccuracy]                         NVARCHAR (500) NULL,
    [FusedAddress]                          NVARCHAR (500) NULL,
    [flgLocationServicesOnOff]              NVARCHAR (500) NULL,
    [flgGPSOnOff]                           NVARCHAR (500) NULL,
    [flgNetworkOnOff]                       NVARCHAR (500) NULL,
    [flgFusedOnOff]                         NVARCHAR (500) NULL,
    [flgInternetOnOffWhileLocationTracking] NVARCHAR (500) NULL,
    [flgStoreOrder]                         NVARCHAR (500) NULL,
    [flgRetailerCreditBalnce]               NVARCHAR (500) NULL,
    [VisitTypeStatus]                       NVARCHAR (500) NULL,
    [flgVisitCollectionMarkedStatus]        NVARCHAR (500) NULL,
    [SelfCreditNote]                        NVARCHAR (500) NULL,
    [MapAddress]                            NVARCHAR (500) NULL,
    [MapCity]                               NVARCHAR (500) NULL,
    [MapState]                              NVARCHAR (500) NULL,
    [MapPinCode]                            NVARCHAR (500) NULL,
    [NoOrderReasonID]                       NVARCHAR (500) NULL,
    [NoOrderReasonDescr]                    NVARCHAR (500) NULL,
    [flgVisitType]                          NVARCHAR (500) NULL,
    [TeleCallingID]                         NVARCHAR (500) NULL,
    [flgOrderCancel]                        NVARCHAR (500) NULL,
    [VisitComments]                         NVARCHAR (500) NULL,
    [flgIsPicsAllowed]                      NVARCHAR (500) NULL,
    [NoPicsReason]                          NVARCHAR (500) NULL,
    [isGeoValidated]                        NVARCHAR (500) NULL,
    [JointVisitID]                          NVARCHAR (500) NULL,
    [jointVisitCode]                        NVARCHAR (500) NULL,
    [flgJointVisit]                         NVARCHAR (500) NULL,
    [flgOnBehalfOf]                         NVARCHAR (500) NULL);

